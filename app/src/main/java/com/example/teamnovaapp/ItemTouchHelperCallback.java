...
public class ItemTouchHelperCallback extends ItemTouchHelper.Callback {
    ...
    private com.example.teamnovaapp.ButtonsState buttonsShowedState = com.example.teamnovaapp.ButtonsState.GONE;
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive)
    {
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE){
            if(buttonsShowedState != com.example.teamnovaapp.ButtonsState.GONE){
                if(buttonsShowedState == com.example.teamnovaapp.ButtonsState.LEFT_VISIBLE) dX = Math.max(dX, buttonWidth);
                if(buttonsShowedState == com.example.teamnovaapp.ButtonsState.RIGHT_VISIBLE) dX = Math.min(dX, -buttonWidth);
        ...
        float buttonWidthWithOutPadding = buttonWidth - 10; float corners = 5;
        View itemView = viewHolder.itemView; Paint p = new Paint();

        buttonInstance = null;

        //오른쪽으로 스와이프 했을때 (왼쪽에 버튼이 보여지게 될 경우)
        if(buttonsShowedState == com.example.teamnovaapp.ButtonsState.LEFT_VISIBLE){
        ...
        }else if(buttonsShowedState == com.example.teamnovaapp.ButtonsState.RIGHT_VISIBLE){
    ...
    private void setTouchListener(final Canvas c, final RecyclerView recyclerView,
        ...
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(swipeBack){
                    if(dX < -buttonWidth) buttonsShowedState = com.example.teamnovaapp.ButtonsState.RIGHT_VISIBLE;
                    else if(dX > buttonWidth) buttonsShowedState = com.example.teamnovaapp.ButtonsState.LEFT_VISIBLE;
                    if(buttonsShowedState != com.example.teamnovaapp.ButtonsState.GONE){
    ...
    private void setTouchUpListener(final Canvas c, final RecyclerView recyclerView , final RecyclerView.ViewHolder viewHolder, final float dX, final float dY , final int actionState, final boolean isCurrentlyActive){
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                com.example.teamnovaapp.ItemTouchHelperCallback.super.onChildDraw(c, recyclerView, viewHolder, 0F, dY, actionState, isCurrentlyActive);
                recyclerView.setOnTouchListener(new View.OnTouchListener() {
                    @Override
                    public boolean onTouch(View v, MotionEvent event) {
                        return false;
                ...
                buttonsShowedState = com.example.teamnovaapp.ButtonsState.GONE; currenrtItemViewHolder = null; return false;
            }
        });
    }
    private void setItemsClickable(RecyclerView recyclerView, boolean isClickable){
        for(int i = 0; i < recyclerView.getChildCount();
            i++){ recyclerView.getChildAt(i).setClickable(isClickable);
        }
    }
}
